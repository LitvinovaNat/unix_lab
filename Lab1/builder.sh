...
executable_file_name=$(sed "$OUTPUT_REGEX" <<< "$filename" | grep -m 1 "")
[ -z "$executable_file_name" ] && error 'Output name is not found or empty'

echo "Create temporary folder..."
mktemp_name=$(mktemp -d) || error 'Failed to create temp folder'
...
current_path=$(pwd)
cd "$mktemp_name"
g++ -o $executable_file_name.exe "${mktemp_name}/${executable_file_name}.cpp"

echo "Move executable file to current path..."
cp "${executable_file_name}.exe" "$current_path" || clean_up 'Failed to move executable file'
